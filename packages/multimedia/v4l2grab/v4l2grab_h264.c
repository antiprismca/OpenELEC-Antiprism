...
/**
  process image read
*/
static int imageProcess(const void* p, int size)
{
...
static int frameRead(void)
{
  ...
  switch (io) {
    ...
    case IO_METHOD_MMAP:
      ...
      ret = imageProcess(buffers[buf.index].start, buf.bytesused);
    ...
    case IO_METHOD_USERPTR:
      ...
      ret = imageProcess((void *) buf.m.userptr, buf.bytesused);
...
static void mainLoop(void)
{
  ...
  count = (pixelformat == V4L2_PIX_FMT_H264 ? 1024 * 1024 * 1024 : 1);
...
static void deviceInit(void)
{
  ...
  fmt.fmt.pix.field       = (pixelformat == V4L2_PIX_FMT_YUYV ? V4L2_FIELD_INTERLACED : V4L2_FIELD_ANY);
...
static void usage(FILE* fp, int argc, char** argv)
{
  fprintf (fp,
    ...
    "-v | --video         Use H.264 if supported with device, output to stdout\n"
...
static const char short_options [] = "d:hvo:q:mruW:H:";
...
int main(int argc, char **argv)
{
  ...
  if (!jpegFilename && pixelformat == V4L2_PIX_FMT_YUYV) {
        fprintf(stderr, "You have to specify JPEG output filename!\n\n");
        usage(stdout, argc, argv);
        exit(EXIT_FAILURE); 
  }
  else if (jpegFilename && pixelformat == V4L2_PIX_FMT_H264) {
        fprintf(stderr, "You have to choose either -v or -o!\n\n");
        usage(stdout, argc, argv);
        exit(EXIT_FAILURE);
  }
